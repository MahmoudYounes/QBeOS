...
extern "C" void JumpUsermode() __attribute__((noreturn));
...
void testPDTEntry() {
  ...
  if (expectedEntry != actual) {
    kprint("FAILED:\0");
    kprintf(" read after write value is %b expected %b \0", *((uint32_t *)pp),
            expectedEntry);
...
kargs *parseBootArgs() {
  ...
  __asm__("mov %0, ebx" : "=b"(argsAddress) :);
...
void printArgsMessage(kargs *args) {
...
void kmain() {
  ...
  // setting up an early console
  setupConsole();

  // parsing all the kernel arguments
  kargs *args;
  args = parseBootArgs();
  if (args == NULL) {
  ...
  // print the kernel args
  printArgsMessage(args);
  ...
  // Systems initialized and we are booted
