...
void APIC::enableAPIC(){
    ...
    if (cpu.IsCPUFeatureSupported(CPUID_FEAT_EDX_APIC)){
        kprint("CPU Supports APIC\n\0");
        supportedAPIC = true;
    }

    if (cpu.IsCPUFeatureSupported(CPUID_FEAT_ECX_X2APIC)){
        kprint("CPU Supports x2APIC\n\0");
        supportedX2APIC = true;
    }

    if (!supportedAPIC){
        panic("APIC is not supported.. your CPU arch is not supported by QBeOS yet\n\0");
    }
    ...
    // bits 12:31 on 32bit arch or 12:63 on 64bit arch constitute APIC base address
    initialRegistersAddress = eax & 0xfffff000;
    isBSP = eax & 0x100;
    kprintf(buf, "Found APIC. base addr %x has x2apic %d is BSP %d\n\0", initialRegistersAddress, supportedX2APIC, isBSP);

    // must configure APIC here. IMP!!: no memory writes or apic enable before you virtually map the memory;
