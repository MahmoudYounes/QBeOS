...
func main(){
  ...
  //var outputPath = flag.String("o", "qbeos.img", "the output image file")
	...
	op := "/home/myonaiz/repos/QBeOS/QBeOS.hdd"
	outputPath := &op
  ...
  *bootImagePath, *outputPath)) 
  ...
  if err !=  nil{
    Fatal(fmt.Sprintf("error calculating image size: %s", err.Error()))
  }

	rootEntCnt, err := GetRootEntCnt(*directoryPath)

  fmt.Printf("image size is %d bytes\n", imgSize)
	
	fatImg, err := NewFAT32(IMG_SIZE, rootEntCnt, 0, 2, bootloaderBuffer, secondStageBuffer)
	if err != nil {
	...
	err = fatImg.Serialize(*outputPath)
	if err != nil {
		Fatal(fmt.Sprintf("failed to generate fat image: %s", err.Error()))
	}
